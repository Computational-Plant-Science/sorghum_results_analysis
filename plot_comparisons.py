...
def plot_traits_grid(df, traits=None, cols=2, out_png=None):
    """
    ...
    - out_png (str): Path to save the output image (PNG), if provided.
    """
    df = df.copy()
    ...
    if out_png:
        ...
        print(f"Saved grid plot to {out_png}")
    else:
        fig.show()


def compare_two_locations(df1, df2, traits=None, cols=2, out_png=None):
    """
...
def main():
    ...
    p.add_argument('--out',help='Path to save output image (PNG)')
